...
class Auth extends CI_Controller {
	...
	function activate_account($user_id, $token = false)
	{
		...
		$this->flexi_auth->activate_user($user_id, $token, false);
	...
	function logout() 
	{
		...
		$val = $this->input->get('er');
		// Set a message to the CI flashdata so that it is available after the page redirect.
 		if ($val == "invalid_ip")
 			$this->session->set_flashdata('message', "<p class='error_msg'>Your current IP address does not match your account's assigned IP Address.</p>");
 		else
 			$this->session->set_flashdata('message', $this->flexi_auth->get_messages());
		redirect('auth');
    }
