...
class Demo_auth_model extends CI_Model {
	...
	function register_account()
	{
		...
		$validation_rules = array(
			...
			array('field' => 'register_newsletter', 'label' => 'Newsletter', 'rules' => ''),
			array('field' => 'register_assigned_ip', 'label' => 'Assigned IP', 'rules' => ''),
			array('field' => 'register_email_address', 'label' => 'Email Address', 'rules' => ''),
		...
		if ($this->form_validation->run())
		{
			...
			if ($this->input->post('update_assigned_ip') == "")
				$ip = "::1";
			else
				$ip = $this->input->post('update_assigned_ip');
			// Get user profile data from input.
			// You can add whatever columns you need to customise user tables.
			$profile_data = array(
				'upro_first_name' => ucwords($this->input->post('register_first_name')),
				'upro_last_name' => ucwords($this->input->post('register_last_name')),
			...
			if (!$response)
			{
				$this->data['message'] = '<p class="status_msg">Account information has been successfully inserted.</p>';
				$this->session->set_flashdata('message', '<p class="status_msg">Account information has been successfully inserted.</p>');
				return true;
			}
		}
	...
	function change_password($identity)
	{
		...
		if ($this->form_validation->run())
		{
			...
			($response) ? redirect('auth_admin/manage_user_accounts') : redirect('auth_admin/change_password');
		}
		else
		{	
			// Set validation errors.
